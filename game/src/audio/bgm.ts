...
//
// 음향 방향은 sfx.ts와 같은 언어를 쓴다 — A 마이너 펜타토닉, 사인/삼각파, 완만한 엔벨로프.
// 효과음과 배경음이 같은 음계 안에 있으면 동시에 울려도 불협이 나지 않는다. 이게 펜타토닉을
// 고른 실용적 이유다(효과음은 한 판에 수백 번 배경음 위에 겹친다).
//
// **게인 규율**: 배경음은 반주다. 아래 노트 게인은 **가장 조용한 효과음(tap 0.05)보다도 낮다** —
// 최대가 0.028이다. 버스에 볼륨 배수를 걸어 조절하지 않는 것도 의도다: 두 버스는 유니티 게인이
// 계약이고(버스 삽입으로 음량이 변하면 sfx.ts의 게인 비율이 흔들린다), 음량은 여기서 조율한다.
// 여기 값을 올려 "잘 들리게" 만들려는 유혹이 생기면 그건 배경음이 아니라 전경음이 되고,
// 화면의 이벤트 텍스트와 경쟁한다. 이 부등호를 테스트가 잠근다.
//
// 켜고 끄는 주체가 이 파일이다 — 엔진은 음악을 모른다. 설정 구독으로 mute/bgmEnabled 변화를,
// 준비 구독으로 "컨텍스트가 running이 된 순간"을 받아 start/stop한다. 둘 중 하나라도 없으면
// 음소거를 풀거나 🎵를 눌러도 아무도 다시 켜주지 않는다.
import {
...
const MAIN_THEME: BgmTrack = {
  ...
  notes: [
    ...
    { beat: 0, freq: A2, beats: 7.5, gain: 0.028 },
    { beat: 8, freq: E3, beats: 7.5, gain: 0.025 },
    // 중역 패드 — 화성을 채우되 거의 안 들릴 만큼
    { beat: 0, freq: A3, beats: 7.5, gain: 0.015 },
    { beat: 8, freq: C4, beats: 7.5, gain: 0.014 },
    // 선율 — 삼각파로 살짝 존재감만. 마지막 두 박은 비워 숨을 남긴다.
    { beat: 0.0, freq: A4, beats: 1.5, gain: 0.021, type: 'triangle' },
    { beat: 2.0, freq: C5, beats: 1.0, gain: 0.019, type: 'triangle' },
    { beat: 3.5, freq: E5, beats: 1.5, gain: 0.020, type: 'triangle' },
    { beat: 6.0, freq: D5, beats: 1.0, gain: 0.018, type: 'triangle' },
    { beat: 8.0, freq: C5, beats: 1.5, gain: 0.020, type: 'triangle' },
    { beat: 10.0, freq: A4, beats: 1.0, gain: 0.018, type: 'triangle' },
    { beat: 11.5, freq: G4, beats: 1.5, gain: 0.017, type: 'triangle' },
    { beat: 13.0, freq: E4, beats: 2.0, gain: 0.016, type: 'triangle' },
  ],
};

/** 배경음 앞단 로우패스 — 고역을 깎아 먹먹하고 따뜻하게. 텍스트를 읽는 화면이라 날카로우면 방해된다. */
...
/** sfx.ts의 envelope과 같은 형태 — 다만 지속음이라 어택을 훨씬 길게 준다. */
...
function scheduleLoop(): void {
  ...
  // 컨텍스트가 잠겼거나(탭 이탈로 suspend) 사라졌다 — 조용히 접는다. 복귀는 준비 신호를 받는
  // 구독이 담당한다. 여기서 재시도 루프를 돌리면 잠긴 채로 타이머가 영원히 남는다.
...
export function startBgm(): void {
  ...
  const target = getBgmTarget();
  if (!target) return;      // 아직 running이 아니다 — 준비 신호를 받으면 구독이 다시 부른다
  try {
 ...
 *     탭 복귀(resumeAudioFromBackground)도 같은 신호로 들어와, 백그라운드에서 멈춘 재생이 되살아난다.
 *
...
export function syncBgmWithSettings(): () => void {
  ...
  // startBgm은 재생 중이면 무시하므로 중복 호출은 안전하다.
