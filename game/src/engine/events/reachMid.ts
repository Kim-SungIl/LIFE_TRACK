...
export const MID_REACH_EVENTS = [
  ...
  },
  {
    ...
    title: '또 끝자리',
    description: '게시판에 붙은 학급 단체사진. 수빈은 사진 속 자기를 찾다가 맨 끝, 프레임에 반쯤 잘려 어깨가 걸린 자기를 가리킨다. "또 끝이네." 웃음이 끝까지 가지 못한다.\n"난 맨날 끝이더라. 끼긴 끼는데, 한가운데는 늘 딴 애야."',
    speakers: ['subin'],
    location: 'hallway',
    background: 'hallway_middle',
    condition: (s) => { const n = s.npcs.find(x => x.id === 'subin'); return !!n?.met && n.intimacy >= 78 && s.year === 2 && !s.isVacation; },
    choices: [
      { text: '"끝에 있으면 다 보이잖아. 너 그래서 다 알더라"', effects: { social: 1 }, npcEffects: [{ npcId: 'subin', intimacyChange: 5 }], message: '수빈이 사진을 한참 보다 "…야 너 뭐냐 ㅋㅋ" 하면서 귀가 빨개진다.', timeCost: 1, memorySlotDraft: { category: 'discovery', importance: 7, toneTag: 'melancholy', recallText: '또 끝이라던, 끝이라 다 본다던 말.', npcIds: ['subin'] } },
      { text: '"다음 사진은 내가 네 옆 한가운데로 끌고 갈게"', effects: { social: 1 }, npcEffects: [{ npcId: 'subin', intimacyChange: 4 }], message: '"…해보든가." 수빈이 작게 웃는다.', timeCost: 1 },
      { text: '아무 말 없이 사진 속 수빈 위에 손가락으로 동그라미를 그려준다', effects: { mental: 1 }, npcEffects: [{ npcId: 'subin', intimacyChange: 4 }], message: '수빈이 그 동그라미를 가만히 본다. 한참을.', timeCost: 1 },
    ],
  },
  {
