...
describe('성취 등급 도달 가능성 (7년 완주)', () => {
  ...
  it('무료 성실 플레이는 A에 착지한다 — 시간만으로는 S에 못 닿는다', { timeout: 30_000 }, () => {
    ...
    // #489(방학식 summer-start W20→W19)로 이 두 시드가 84.0→84.1 · 83.0→84.3으로 올라갔다.
    //
    // **다만 "여유가 줄었다"로 읽지 말 것 — 두 시드의 국소 현상이다.** 시드 1~60을 이동 전후로
    // 돌려 재면 분포는 거의 그대로다: 평균 83.82 → 83.77(오히려 내려감), 85 이상이 3/60 → 5/60.
