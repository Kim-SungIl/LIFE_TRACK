...
//   7년을 돌려보면 공부형 빌드에서도 NPC 10명이 전원 절친(≥70)에 도달한다. 친밀도로는 빌드가
//   구분되지 않는다. 그런데 도달형 이벤트 96종은 학년(reach.year)으로 게이트돼 Y1~Y7에 고르게
//   흩어져 있어서, 고3에 절친이 돼도 중1 이야기는 영영 안 열린다.
//   실제로 공부형은 121종, 관계형은 185종을 보고 그 격차의 92%가 NPC 이벤트다.
...
// "거의 스치기만 한" 기준. 첫 판(공부형) 실측에서 서아 1/9·시우 1/8·예린 1/6이 여기 걸리고
// 도윤 3/7·수빈 13/32는 안 걸린다 — 다음 판에 실제로 가볼 만한 곳만 남기는 선.
