...
export const ACTIVITIES: Activity[] = [
  ...
  // === 자기계발 계열 ===
  {
    // T24: 피로 6 → 5. 유료 상위 활동의 설계 규약("효율↑ 피로↓를 돈으로 산다" — 독서실 2.0/f4
    ...
    // 실효 지렛대는 피로였다: 신고 조건 18주의 주당 증가가 1.90 → 0.70으로 무너지는데,
    // 그 원인이 누적 피로와 그로 인한 멘탈 하락이다.
    //
    // 5로 내리면 역전이 풀린다(1칸 41.8 vs 40.1 · 2칸 57.3 vs 54.9). 피로 계단에서 5는
    // 독학·코딩 독학이 쓰는 칸이고, 무료 창작(3)보다는 여전히 위라 "전문 레슨은 더 고되다"는
    // 결도 유지된다. Y6 상위 활동과의 순서도 그대로다(입시 실기 74.2 > 예체능 71.5 > 창작 67.4).
    id: 'art-lesson', name: '예체능 레슨', slots: 1, fatigue: 5,
