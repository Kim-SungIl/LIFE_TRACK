...
export const ACTIVITIES: Activity[] = [
  ...
  {
    // T39: 피로 7 → 4. T24가 예체능 레슨에서 잡은 규약("유료는 무료/하위 대안의 상위 — 효율↑ 피로↓를
    // 돈으로 산다")을 그 **상위 활동**이 그대로 어기고 있었다. 예체능 레슨(talent 2.0/f4/2만)보다
    // 값은 0.8 큰데 피로가 +3이라, 2배를 내고 더 지치는 쪽이었다.
    //
    // T24는 이 짝을 "Y6 18주에서 위"(입시 실기 74.2 > 예체능 71.5)로 재 두고 넘어갔다. 그 값은
    // **특기가 낮아 주당 축 상한(+2)이 안 물리던 구간**의 값이다. 특기가 80을 넘으면 감쇠가 값을
    // 2 아래로 끌어내려 상한이 풀리고, 그때부터는 피로 프리미엄만 남는다.
    //
    // 7년 96시드 격리 실측(루틴 인강+가벼운운동 고정 · 주말 슬롯0만 Y6부터 교체 · 주말 슬롯1 동아리):
    //   예체능 f4(2만)  수능 90.49 · 등급 2.04 · SKY 19/96 · 특기 96.8 · 지침 0.0% · 잔액 1135만
    //   실기 f7(현행)   수능 87.47 · 등급 2.64 · SKY  0/96 · 특기 96.7 · 지침 1.4% · 잔액  986만
    //   실기 f6         수능 88.23 · 등급 2.49 · SKY  4/96 · 특기 97.5
    //   실기 f5         수능 89.70 · 등급 2.24 · SKY 16/96 · 특기 98.5
    //   실기 f4(확정)   수능 90.64 · 등급 1.98 · SKY 22/96 · 특기 99.4 · 지침 0.0%
    // f7은 **산 축에서도 졌다**(특기 96.7 vs 96.8). 특기 빌드(학업 83)에서는 진로가 더 노골적이었다:
    // 인서울 상위권 32/96 → 2/96 · SKY 2 → 0.
    //
    // 5가 아니라 4인 이유는 T28이 헬스에서 쓴 기준 그대로 "시드 노이즈 대역 안으로 들어오는 첫 값"이다.
    // f5는 −0.79점 · SKY 16 vs 19인데, 같은 배치를 **다른 시드 블록(201~296)** 으로 다시 재면
    // 예체능 90.21 · SKY 18, 실기 f5 89.26 · SKY 7로 격차가 오히려 커진다(노이즈는 ±0.4점 수준).
    // f4에서 비로소 수능·SKY·특기가 전부 위가 된다 — 학원 f4 = 인강 f4와 같은 결론으로,
    // **대안과 같은 칸**이 이 짝의 최소 조건이다. 값(2.8)·가격(4만)은 한 글자도 안 건드렸다.
    id: 'practical-lesson', name: '입시 실기 레슨', slots: 1, fatigue: 4,
  ...
  // 유료 3개
  {
    // T39: 피로 12 → 5. 유료 상위 규약("효율↑ 피로↓를 돈으로 산다")을 방학 칸에서 어기고 있었다.
    // 방학 도서관 몰입(academic 3 + mental 1 / f3 / 무료 / 2칸)과 **효과 합이 4로 같은데**
    // 피로가 4배였다. 같은 2칸을 놓고 다투는 짝이라 슬롯당으로 재면 6 vs 1.5다.
    //
    // 7년 96시드 격리 실측(방학 5칸 중 앞 2칸만 교체 · vacationLimit 2에 닿으면 양쪽 다
    // 방학 도서관으로 넘어간다 = 제품에서 그 칸에 실제로 들어갈 것). 방학 나머지 3칸을
    // **휴식 0칸**(독학+동아리+도서관자습)으로 둔 배치, **시드 블록 두 벌**(1~96 / 201~296):
    //   방학도서관 f3   멘탈 82.0 / 82.0 · 행복 S 88 / 90 · 지침 1.7 / 2.0%
    //   단기특강 f12    멘탈 79.2 / 79.2 · 행복 S 16 / 18 · 지침 2.3 / 2.7%   ← 현행
    //   단기특강 f9     멘탈 79.7 / 79.6 · 행복 S 37 / 27
    //   단기특강 f7     멘탈 80.2 / 80.1 · 행복 S 44 / 47
    //   단기특강 f5     멘탈 80.5 / 80.5 · 행복 S 61 / 57 · 지침 1.8 / 2.1%   ← 확정
    //   단기특강 f4     멘탈 81.0 / 80.8 · 행복 S 69 / 66
    // 학업은 반대로 f12가 +0.6 앞선다(91.4 vs 90.8). 즉 **산 축 하나를 사고 행복 등급을 내준다.**
    //
    // **판정 지표를 수능 점수로 잡으면 안 된다.** 이 배치의 수능은 블록 간에 흔들린다 —
    // 같은 무료 배치가 86.98 → 86.35이고 f12와의 격차가 1.58 → 0.33으로 줄었다. 휴식 1칸짜리
    // 대표 배치도 마찬가지다(무료 89.07 · SKY 11 vs f12 88.09 · SKY 3 이던 것이 블록2에서
    // 88.79 · SKY 7 vs 88.73 · SKY 4). 반면 **멘탈과 행복 등급은 두 블록에서 소수점까지 같다.**
    // 그래서 이 짝의 근거는 멘탈·행복 등급·지침이고, 수능/SKY는 판정 불가로 둔다.
    //
    // **휴식 2칸으로 채운 방학에서는 아무 차이도 안 난다**(수능 89.40 vs 89.44 · 행복 전부 S) —
    // T28이 주말에서 겪은 상쇄 함정과 같은 것이라, 그 배치로 재고 "문제 없다"고 하면 안 된다.
    //
    // 5인 이유: 같은 2칸끼리의 짝이므로 **슬롯당** 프리미엄 1칸 = 총 +2가 규약이 허용하는 최대이고,
    // 그게 곧 f5다. 피로를 더 내려도(f4) 행복 S가 61 → 69로 8칸 더 갈 뿐 무료(88)에는 못 닿는다 —
    // 남는 격차는 피로가 아니라 **무료 쪽이 들고 있는 mental +1**이고, 값은 이 조정의 범위가 아니다.
    // 값(academic 4)·가격(5만)·방학당 2회 제한은 한 글자도 안 건드렸다.
    id: 'intensive-academy',
    name: '학원 단기특강', slots: 2, fatigue: 5,
