...
export function NpcSelectModal({ metNpcs, year, npcSelectFor, onSelect, onCancel }: Props) {
  ...
  return (
    <Dialog onClose={onCancel} labelledBy="npc-select-title" maxWidth={400}
      contentStyle={{ background: 'var(--bg-secondary)', borderRadius: 16, padding: 24 }}>
      <div id="npc-select-title" style={{ fontSize: '1rem', fontWeight: 700, marginBottom: 4 }}>누구와 {modalLabel}?</div>
      <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', marginBottom: 16 }}>함께하는 친구에 따라 친밀도가 올라갑니다</div>
      {metNpcs.map(npc => (
        <button key={npc.id} type="button" className="btn-reset" onClick={() => onSelect(npc.id)}
          style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: 'var(--bg-card)', borderRadius: 10, marginBottom: 6, cursor: 'pointer', border: '1px solid transparent', transition: 'all 0.2s', width: '100%' }}
