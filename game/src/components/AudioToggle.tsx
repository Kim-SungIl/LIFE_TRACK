...
//
// 볼륨 슬라이더는 여전히 넣지 않는다. 효과음·배경음 모두 저음량으로 설계돼 있어 실사용
// 선택지는 "켠다/끈다"뿐이고, 세밀한 조절은 OS·브라우저 볼륨으로 충분하다. 슬라이더를 붙이면
// 크롬 하나 때문에 헤더가 설정 화면이 된다.
//
// **버튼이 둘인 이유**: 음소거는 전체(효과음+배경음)이고 배경음은 그 안의 선택이라, 하나로
// 합치면 "효과음만 켜기"가 불가능해진다. 지속음을 싫어하지만 효과음은 원하는 경우가 흔하다.
// 배경음은 기본 꺼짐이라 대부분의 사용자에게 🎵는 흐린 상태로 남는다.
