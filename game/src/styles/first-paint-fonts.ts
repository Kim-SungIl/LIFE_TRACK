...
export const FIRST_PAINT_TEXT = [
  ...
  '기록실',
  '지금까지의 학창시절',
  // 직전 판 설정이 있음 — "새 게임" 버튼의 서브라벨(TitleScreen, 다회차 입구).
  // **이 줄이 늦게 들어온 이유**: 이 텍스트를 넣은 PR(#440)의 테스트 픽스처가 세이브만 심고
  // lifetrack_last_setup 키는 안 심어서, 게이트가 도는 화면에 이 서브라벨이 아예 없었다.
  // 실측으로는 조각이 9개 그대로다(82,83,84,86,87,88,89,90,91) — 글리프가 이미 고른 조각
  // 안에 있어서 preload 비용은 0이다. 그래도 상수가 화면을 설명하지 못하면, 서브셋을 다시
  // 생성해 매핑이 바뀌는 날 이 글자들이 조용히 조각 밖으로 나간다.
  '같은 집에서 다시 / 처음부터',
