...
// ============================================================================
{
  ...
  assert('types.ts에 talkEventPressure/parentIntimacy/talkEventsFired 필드',
    /talkEventPressure:\s*number/.test(typesSrc)
    && /parentIntimacy:\s*number/.test(typesSrc)
    && /talkEventsFired:\s*string\[\]/.test(typesSrc));
  assert('createInitialState/migrateLoadedState 모두 새 필드 초기화',
    /talkEventPressure:\s*0/.test(engineSrc)
    && /parentIntimacy:\s*50/.test(engineSrc)
    && /talkEventPressure:\s*state\.talkEventPressure\s*\?\?\s*0/.test(engineSrc));
  assert('processWeek가 매주 pressure 증가(+0.1/+0.05) + 플래그 리셋',
    /talkEventPressure\s*=\s*Math\.min\(1,.*\+\s*0\.1/.test(engineSrc)
    && /parentTalkPressure\s*=\s*Math\.min\(1,.*\+\s*0\.05/.test(engineSrc)
    && /weekTalkRolledForNpc\s*=\s*false/.test(engineSrc));
  assert('store.ts에 talkToNpc/talkToParent 액션 + 미니 이벤트 1회 보장',
    /talkToNpc:\s*\(npcId\)\s*=>/.test(storeSrc)
    && /talkToParent:\s*\(strength\)\s*=>/.test(storeSrc)
    && /talkEventsFired\s*=\s*\[\.\.\..*talkEventsFired,/.test(storeSrc));
  ...
  assert(`NPC 미니 이벤트 풀: 6 NPC 커버 (현재 ${NPC_MINI_EVENTS.length}개)`,
