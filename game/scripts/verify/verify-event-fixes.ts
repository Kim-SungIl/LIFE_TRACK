...
console.log('\n=== 3. friend-snack: minjae.met 조건 (소스 검증) ===');
{
  ...
  // P0-2 분리 이후 events 는 디렉토리 — 하위 모든 .ts 텍스트 재귀 합산
  const EVENTS_DIR = path.join(__dirname, '..', '..', 'src', 'engine', 'events');
  const readAllEventsText = (dir: string): string => {
    const out: string[] = [];
  ...
  const src = readAllEventsText(EVENTS_DIR);
...
console.log('\n=== 12. 상점 buff 중복 구매 차단: 남은 기간 손실 방지 ===');
{
