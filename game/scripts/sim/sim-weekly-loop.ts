 ...
 *
 * 실행: npx tsx scripts/sim/sim-weekly-loop.ts [시드수]
 */
...
function runRoutine(r: Routine, seed: number) {
  ...
  for (let week = 0; week < 420 && s.year <= 7; week++) {
    ...
    // 돈 부족 스킵은 엔진이 카운터를 안 남긴다 — 주간 로그 문구가 유일한 신호다
    // (applyRoutineActivities / applyWeekendActivities 둘 다 같은 문구를 push한다).
    {
      const msgs = (s.weekLog?.messages ?? []).filter(m => m.includes('돈이 부족해서'));
      if (msgs.length > 0) years[y].skipped++;
      // 루틴과 선택 슬롯이 같은 문구를 쓰므로 활동 이름으로 가른다(그래서 루틴과 주말에
      // 서로 다른 활동을 쓰는 페르소나로만 이 분해가 유효하다 — 위 ROUTINES가 그렇다).
      const routineNames = [s.routineSlot2, s.routineSlot3]
        .map(id => ACTIVITIES.find(a => a.id === id)?.name).filter((n): n is string => !!n);
      if (msgs.some(m => routineNames.some(n => m.includes(n)))) years[y].skipRoutine++;
      if (msgs.some(m => !routineNames.some(n => m.includes(n)))) years[y].skipChoice++;
    }
  ...
  return { years, everAvailable, firstSeenYear };
}
...
function main() {
  ...
  for (const r of ROUTINES) {
    ...
    for (let seed = 1; seed <= seeds; seed++) {
      const { years, everAvailable, firstSeenYear } = runRoutine(r, seed * 1000 + 7);
