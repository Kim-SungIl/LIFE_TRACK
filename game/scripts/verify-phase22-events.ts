...
const NEW_EVENT_SPECS: Array<{ id: string; npcId: string; threshold: number }> = [
  ...
  { id: 'yuna-window-promise', npcId: 'yuna', threshold: 90 },
  ...
  { id: 'subin-paper-airplane', npcId: 'subin', threshold: 90 },
...
function makeState(npcId: string, intimacy: number, met: boolean, year: number = 1): GameState {
...
for (const spec of [...NEW_EVENT_SPECS, RETOOLED_DOYUN_COMIC]) {
  ...
  const s = makeState(spec.npcId, spec.threshold, true);
  const result = ev.condition(s);
  check(`  ${spec.id} (intimacy=${spec.threshold}) fires`, result === true);
}
