...
> ⚠️ `classroom_high_afternoon.png`는 **실재하지 않음**(asset 확인). 촬영 리스트가 일부 컷 배경으로 적은 `classroom_high_afternoon`은
> `classroom_high.png`(주간) 또는 `classroom_high_sunset.png`(석양)로 대체. 본 문서는 대체 배경을 병기.
...
Back view or 3/4 from behind so the figure works gender-neutrally — show tension in the
shoulders, not the face. Bare feet / house slippers, indoor loungewear (long-sleeve
...
shadowed (gender-neutral framing). Single desk lamp pooling warm light over the book,
...
Protagonist shown from behind or in soft profile (gender-neutral); mother's hand resting
...
a wall, looking up at the vast distant sky. Back view / face turned up and away (gender-neutral).
...
The protagonist (uniform, 3/4 or side view, gender-neutral) talking to a homeroom teacher —
...
night, phone to ear, faint smile breaking through; warm bedroom/rooftop night lighting.
...
(3/4 / partly back, gender-neutral) sharing the moment. Casual trip clothes (light jacket /
...
```
Empty high-school classroom in the afternoon (classroom_high.png, or classroom_high_sunset.png
for melancholy tone — note: classroom_high_afternoon.png does not exist). A single protagonist
(uniform, back view / face hidden, gender-neutral) at a desk with a study book/workbook open,
...
academy registration slip ("학원 등록증" / enrollment paper, generic — no brand). Close on the
hands and paper, the student's determined face partly in frame. Warm hagwon lobby lighting,
...
from doorway / gender-neutral) stepping in, hand on the door frame, facing an awkward but warm
...
their reflection), gender-neutral. On a nearby surface: a framed school-campus photo / a few
...
3/4 / back, gender-neutral) facing her. Soft pools of streetlamp light, blue evening dusk around,
...
```
Same hagwon-front streetlamp scene, evening. Subin (subin_high reference) and protagonist
standing a little apart under the lamp, the goodbye lingering — Subin with a softer, wistful
...
one. Eyes a little glassy with resolve. The protagonist (uniform, 3/4 / back, gender-neutral) before
...
casually) and the protagonist (uniform, side/3/4, gender-neutral) standing side by side at the
...
turning toward the protagonist, his bright grin faltering — eyes reddening, brimming, the energetic
boy moved to the edge of tears. The protagonist facing him (3/4 / back, gender-neutral). Wind,
...
gender-neutral) facing him, an encouraging air. Junha's face proud and a little shy as he
...
```
Same rooftop lunch scene (rooftop / rooftop_sunset reference). Junha (junha_high reference)
breaking into a wide, genuinely moved smile — the awkward transfer student fully at ease now —
as the protagonist promises to be the first customer at his future restaurant. The open home-made
...
out rather than at the protagonist. The protagonist (uniform, 3/4 / back, gender-neutral) beside
...
17-year-old protagonist in a crisp new high-school uniform (navy blazer + navy tie + chest crest)
...
> 선택지 순서(코드 확인): **c0 = 문과**, **c1 = 이과**, c2 = (보류/공통). 두 갈래 CG 또는 공통 1장.
...
handed out, students murmuring. The protagonist (uniform, 3/4 / back, gender-neutral) holding the
...
```
Same classroom track-selection scene (classroom_high reference). Protagonist (uniform,
gender-neutral) marking the SCIENCE (이과) choice, pen on the science track line, determined.
...
mood. A "D-xxx" countdown to 수능 written above the blackboard. Students at desks, subdued.
Protagonist (uniform, 3/4 / back, gender-neutral) seated, facing forward toward the blackboard
...
deep-night window. The protagonist (loungewear, late-autumn night; 3/4 / back, gender-neutral)
...
Protagonist (uniform or casual, 3/4 / back, gender-neutral) at the table. Warm restaurant lighting,
...
back view, gender-neutral) walking out into the open light, the long road of 12 years behind them.
Wide cinematic sky.
...
protagonist (high-school uniform, navy tie + chest crest, 3/4 / back, gender-neutral) surrounded
...
late afternoon. The blackboard reads "졸업 축하해. 우리 모두 수고했어." Empty desks, soft light.
Protagonist (uniform, back view, gender-neutral) taking one last look at the room.
...
3/4 / back, gender-neutral) at a desk, listening, a knot of nerves. Daylight through windows.
...
Protagonist (uniform, 3/4 / back, gender-neutral) at a desk, the gravity of college admissions
...
gold star earrings, navy blazer high-school uniform) and the protagonist (uniform, 3/4 /
gender-neutral) putting up a promo poster together, a shared determined glance between them.
...
```
A dim quiet room (bedroom or empty classroom), the protagonist (back view / lying back,
gender-neutral) staring at the ceiling, a phone glowing with a group-chat full of festival
...
protagonist (uniform, 3/4 / gender-neutral) explains it, pointing at the workbook. Open books,
...
```
Same high-school library (library_high reference). Yuna (yuna_high reference) turning away
lightly, a small disappointed-but-okay shrug, her back to the protagonist as she walks off
between the shelves. Protagonist (uniform, gender-neutral) at the desk, the refusal lingering.
